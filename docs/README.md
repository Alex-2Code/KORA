...
Current runnable flows in the public repository:
...
`telemetry` can be run immediately with the committed sample input above, or with a JSON artifact generated by a prior run or report flow.

Recommended public flow:

1. First run: `examples list`, `hello_kora`, `retry_demo`, and `direct_vs_kora -- --offline`.
2. Telemetry: summarize `docs/reports/sample_telemetry_input.json`.
3. Next step: move to `docs/benchmark-real-app.md` and `examples/real_workload_harness/`.
