...
The repository-level preview also exposes `POST /api/harness/run` for approved local deterministic sample request IDs only, `GET /api/harness/run/<run_id>` for in-memory run retrieval, and `GET /api/harness/events?run_id=<id>` for generated event retrieval. The events endpoint is not SSE, not model token streaming, and not model output. Arbitrary prompt execution is not connected. Model-needed boundaries return `execution_not_connected` and do not execute a model. Provider calls, cloud sync, downloads, runtime model listing, and private model directory scanning remain disabled.
